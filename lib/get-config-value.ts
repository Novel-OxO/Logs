...
try {
  if (process.env.NEXT_PUBLIC_SITE_CONFIG) {
    siteConfigOverrides = JSON.parse(process.env.NEXT_PUBLIC_SITE_CONFIG) as SiteConfig;
  }
} catch (err) {
...
export function getEnv<T>(key: string, defaultValue?: string | T, env = process.env): string | T {
  const value = env[key];
